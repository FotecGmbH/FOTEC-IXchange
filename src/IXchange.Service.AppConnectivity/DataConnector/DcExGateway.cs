...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public async Task<DcListStoreResult> StoreDcExGateways(long deviceId, long userId, List<DcStoreListItem<ExGateway>> data, long secondId)
    {
        if (data == null!)
        {
            throw new ArgumentNullException($"[{nameof(StoreDcExGateways)}]({nameof(data)}): {nameof(data)} is null");
        }
        ...
        foreach (var gw in data)
        {
            switch (gw.State)
            {
                case EnumDcListElementState.New:
                    var dbGatewayN = new TableGateway();
                    // ReSharper disable once RedundantSuppressNullableWarningExpression
                    gw.Data.ToTableGateway(dbGatewayN!);
                    db.TblGateways.Add(dbGatewayN);
                    r.ElementsStored++;
                    break;
